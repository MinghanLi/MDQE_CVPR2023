...
class YTVISEvaluator(DatasetEvaluator):
    ...
    def _eval_predictions(self, ytvis_results):
        """
        ...
        if self._output_dir:
            ...
            with PathManager.open(file_path, "w") as f:
                f.write(json.dumps(ytvis_results))
                f.flush()
        ...
        task = "segm"
        ytvis_eval = _evaluate_predictions_on_ytvis(self._ytvis_api, ytvis_results, task=task)
...
def instances_to_coco_json_video(inputs, outputs):
    ...
    for instance_id, (s, l, m) in enumerate(zip(scores, labels, masks)):
        segms = [
            mask_encode(np.array(_mask[:, :, None], order="F", dtype="uint8"))[0]
            for _mask in m
        ]
        for rle in segms:
