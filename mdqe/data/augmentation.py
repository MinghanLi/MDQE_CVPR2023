"""
This part comes from SeqFormer (https://github.com/wjf5203/SeqFormer) and VITA (https://github.com/sukjunhwang/VITA).
"""
import numpy as np
import logging
import sys
            ...
            if cfg.INPUT.CROP.ENABLED:
                crop_aug = RandomApplyClip(
                    T.AugmentationList([
                        ...
                        RandomCropClip(cfg.INPUT.CROP.TYPE, cfg.INPUT.CROP.SIZE, clip_frame_cnt=clip_frame_cnt)
                    ]),
...
def build_pseudo_augmentation(cfg, is_train):
    ...
    aug_list = []
    if is_train:
        use_lsj = cfg.INPUT.LSJ_AUG.ENABLED  # Not used in MDQE
        if use_lsj:
            image_size = cfg.INPUT.LSJ_AUG.IMAGE_SIZE
            min_scale = cfg.INPUT.LSJ_AUG.MIN_SCALE
            max_scale = cfg.INPUT.LSJ_AUG.MAX_SCALE
            ...
            if cfg.INPUT.PSEUDO.CROP.ENABLED:
                crop_aug = RandomApplyClip(
                    T.AugmentationList([
                        ResizeShortestEdgeClip([400, 500, 600], 1333, sample_style, clip_frame_cnt=clip_frame_cnt),
                        RandomCropClip(cfg.INPUT.PSEUDO.CROP.TYPE, cfg.INPUT.PSEUDO.CROP.SIZE,
                                       clip_frame_cnt=clip_frame_cnt)
                    ]),
                    clip_frame_cnt=clip_frame_cnt
                )
