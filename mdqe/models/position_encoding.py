...
class CPB(nn.Module):
    ...
    def __init__(self, dim_in=2, dim=32, dim_out=1, depth=2, log_space=False):
        super().__init__()
        '''
        values are slightly fluctuation in log space
        '''
        self.log_space = log_space

        self.mlp = []
        for d in range(depth-1):
            dim_in = dim_in if d == 0 else dim
            self.mlp.append([
                nn.Linear(dim_in, dim),
                nn.ReLU()
            ])

        self.mlp.append([nn.Linear(dim, dim_out)])
        self.mlp = nn.Sequential(*sum(self.mlp, []))
    ...
    def forward(self, rel_pos):
        ...
        bias = torch.sign(rel_pos) * torch.log(rel_pos.abs() + 1) if self.log_space else rel_pos
        bias = self.mlp(bias)
